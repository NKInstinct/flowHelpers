...
# What if we just don't do the scaling? It's all fake anyway...
  
  # Trying to resolve the hanging problem with futures??? Going to do this
  # synchronously for now - it needs the feat and regress values from above and
  # is needed for the very next process so it doesn't really make sense to
  # multi-thread this.
  
  # Only thing I'm not sure of is the copy-on-modify semantics here. I'm going
  # to first try it by assigning the future to seurat and then use seurat in the
  # following PCA call to "unfuture" it, but if that doesn't work I'll pop the
  # future out and then back in as a temp variable.
  ...
  seuratFinal <- future::value(seurat)
